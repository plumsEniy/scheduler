...
@Slf4j
@Component
public class SparkVersionLockEventHandler extends AbstractTaskEventHandler {
    ...
    public boolean executeTaskEvent(TaskEvent taskEvent) throws Exception {
        ...
        String msg = String.format("job id of %s spark version will lock.", jobId);
        logPersist(taskEvent, msg);
        saveJobProps(jobId, executionNode.getId());

        boolean isSuccess = sparkManagerService.lockSparkJobVersion(jobId, true);
        msg = "spark version lock : " + (isSuccess ? "SUCCESS" : "FAILURE");
        logPersist(taskEvent, msg);
        return isSuccess;
    }
    ...
    public EventTypeEnum getHandleEventType() {
        return EventTypeEnum.SPARK_VERSION_LOCK_EXEC_EVENT;
    }
}
