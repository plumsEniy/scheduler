...
@Slf4j
@Component
public class PrestoToPrestoOffExpandWaitEventHandler extends AbstractTidePrestoWaitEventHandler {
    @Override
    protected boolean checkPodCount(TaskEvent taskEvent, List<PodInfo> workPodList) {
        ...
        int remainPod = prestoTideExtFlowParams.getSinkRemainPod();
        int currentPod = prestoTideExtFlowParams.getSinkCurrentPod();
    ...
    protected Long getClusterId(TaskEvent taskEvent) {
        Long flowId = taskEvent.getFlowId();
        PrestoToPrestoTideExtFlowParams prestoTideExtFlowParams = executionFlowPropsService.getFlowExtParamsByCache(flowId, PrestoToPrestoTideExtFlowParams.class);
        return prestoTideExtFlowParams.getSinkClusterId();
    }

    @Override
    public EventTypeEnum getHandleEventType() {
        return EventTypeEnum.PRESTO_TO_PRESTO_TIDE_OFF_WAIT_EXPANSION_POD;
    }
