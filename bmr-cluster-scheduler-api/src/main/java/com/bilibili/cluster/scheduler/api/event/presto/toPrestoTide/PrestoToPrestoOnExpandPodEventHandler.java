...
@Slf4j
@Component
public class PrestoToPrestoOnExpandPodEventHandler extends AbstractTidePrestoDeployEventHandler {

    @Override
    public EventTypeEnum getHandleEventType() {
        return EventTypeEnum.PRESTO_TO_PRESTO_TIDE_ON_POD_EXPANSION;
    }
    ...
    protected Integer getNeedScalePodCount(TaskEvent taskEvent) {
        ...
        return params.getSourceCurrentPod();
    }
    ...
    protected Long getComponentId(TaskEvent taskEvent) {
        Long flowId = taskEvent.getFlowId();
        PrestoToPrestoTideExtFlowParams params = executionFlowPropsService.getFlowExtParamsByCache(flowId, PrestoToPrestoTideExtFlowParams.class);
        return params.getSourceComponentId();
    }
    ...
    protected Long getClusterId(TaskEvent taskEvent) {
        Long flowId = taskEvent.getFlowId();
        PrestoToPrestoTideExtFlowParams params = executionFlowPropsService.getFlowExtParamsByCache(flowId, PrestoToPrestoTideExtFlowParams.class);
        return params.getSourceClusterId();
    }

    /**
     * presto节点快速缩容,仅在阶段2执行
     * @param taskEvent
     * @return
     */
    @Override
