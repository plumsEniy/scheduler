...
@Slf4j
@Component
public class HboJobParamsDeleteFlowPrepareGenerateFactory implements FlowPrepareGenerateFactory {
    ...
    public List<FlowDeployType> fitDeployType() {
        return Arrays.asList(FlowDeployType.HBO_JOB_PARAM_RULE_DELETE);
    }

    @Override
    public List<ResolvedEvent> resolvePipelineEventList(PipelineParameter pipelineParameter) throws Exception {
        PipelineFactory pipelineFactory = FactoryDiscoveryUtils.getFactoryByIdentifier(Constants.HBO_JOB_PARAMS_DELETE_DEPLOY_FACTORY_IDENEITFY, PipelineFactory.class);
